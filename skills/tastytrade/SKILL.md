...
| `get_market_metrics` (detail="full") or `get_positions` | `tt-calendar-expiry-alert.py` | ⚠️ "Calendar [symbol] front leg expires [date] — close the spread before market close to avoid pin risk" for any open calendar whose front (short) leg is ≤ 1 DTE (today or tomorrow); silent otherwise; reads open positions from `/tmp/tt_positions.json` |
