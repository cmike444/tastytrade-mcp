...
| `get_market_metrics` (detail="full") or `get_positions` | `tt-calendar-expiry-alert.py` | ⚠️ "Calendar [symbol] front leg expires [date] — close the spread before market close to avoid pin risk" for any open calendar whose front (short) leg is ≤ 1 DTE (today or tomorrow); silent otherwise; reads open positions from `/tmp/tt_positions.json`. **Covers equity-option calendars and futures-option calendars** (e.g. `./ESM6 EW1M6 260515C4800`). Cross-contract calendars (e.g. short `./ESM6` leg / long `./ESU6` leg) are correctly detected — the hook identifies the short leg as the front regardless of contract month. **Also injected proactively into the morning bundle as `calendar_expiry_alerts` (top-level key, non-empty list only) so the warning is visible at session start before any other trade decisions.** |
| `get_options_greeks` | `tt-compute-skew.py` | 25Δ put/call IV; skew (P−C); directional label; momentum-skew signal if extreme (>5%) |
| `get_options_greeks` | `tt-fetch-earnings-straddle.py` | Auto-writes `/tmp/tt_earnings_moves.json`: `implied_move = (ATM_call + ATM_put) / stock_price` for each underlying in the response. `stock_price` is taken from the `underlying-price` field when present; otherwise estimated via put-call parity `(K + call − put)`. ATM selected by call delta closest to 0.50; falls back to median strike when delta absent. Expiry chosen from calendar front_expiry if available, else nearest future expiry with a complete call+put pair. Merges with any existing entries. Consumed by `tt-ff-exit-monitor.py` for ex-earn FF computation. Also callable standalone (`--fetch UNDERLYING EXPIRY_ISO`) for proactive prefetch using the TastyTrade REST API + session cache. |
